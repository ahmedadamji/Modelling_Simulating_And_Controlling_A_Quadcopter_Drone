...
%         plt_zdot
    ...
    methods
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function obj = Drone_q3c(axis1, axis2, axis3, spaceDim, num_drones)
            if nargin > 1
                obj.axis = axis1;
...
%                 obj.plt_zdot = [0];
                ...
                obj.wind_field_and_terbulance = zeros(3,1);
...
%             hold(obj.posdot_axis,'off');
        ...
        function obj = change_pos_and_orientation(obj)
            ...
            if(~obj.pos(3)==0)
                %Adding this function as there will be no wind or
                % turbulance affecting the trajectory when the quadcopter is on the ground
                add_wind_and_turbulance();
            end
            obj.posdot = obj.posdot + obj.wind_field_and_terbulance  + obj.dt * a;
...
%                     (sin_phi*sin_theta) (cos_phi*sin_theta) (cos_theta)];
            ...
            function add_wind_and_turbulance()
                % Calculating net velocity V
                V = norm(obj.posdot);

                %% Calculating mean wind field
                u20 = 0.0004; % mean wind field is described by this, testing 
                            % with different values.
                h = obj.pos(3);

                %mean_wind_field = [0;0;0];
            
                mean_wind_field = [u20*(log(obj.pos(3)/0.15)/log(20/0.15)); 0; 0];
                %disp(mean_wind_field);
                
                %% Calculating turbulent wind:

                % to calculate the turbulence scale lengths Lu, Lv and Lw and 
                % intensities σu, σv and σw as per Eqn 14 in
                % https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=6943033
%                 L_w = -obj.pos(3);
%                 L_v = -obj.pos(3)/(0.177 + 0.000823*h)^1.2;
                L_w = obj.pos(3)/2; %https://iopscience.iop.org/article/10.1088/1742-6596/1005/1/012017/pdf
                L_v = obj.pos(3)/(0.177 + 0.000823*h)^1.2;
                L_u = L_v;

                %disp([L_u,L_v,L_w]);
                
                %w20 is the turbulance level.
                w20 = 0.0004;
                S_w = 0.1*w20; %https://iopscience.iop.org/article/10.1088/1742-6596/1005/1/012017/pdf
                S_u = S_w/((0.177 + 0.000823*h)^0.4);
                S_v = S_u;
                
                % To calculate white noise perturbations with variances
                % calculated above according to 
                % https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=6943033
                E_u = normrnd(0,S_u);
                E_v = normrnd(0,S_v);
                E_w = normrnd(0,S_w);

                %disp(obj.wind_field_and_terbulance);

                T_u = ((1 - (V*obj.dt/L_u))*obj.wind_field_and_terbulance(1)) + (E_u*sqrt(2*V*obj.dt/L_u));
                T_v = ((1 - (V*obj.dt/L_v))*obj.wind_field_and_terbulance(2)) + (E_v*sqrt(2*V*obj.dt/L_v));
                T_w = ((1 - (V*obj.dt/L_w))*obj.wind_field_and_terbulance(3)) + (E_w*sqrt(2*V*obj.dt/L_w));
                turbulent_wind = [T_u;T_v;T_w];
                disp(turbulent_wind);
            
                %% Calculating total wind
                obj.wind_field_and_terbulance = (mean_wind_field + turbulent_wind)';
                %disp(obj.wind_field_and_terbulance);
                %obj.wind_field_and_terbulance = [0;0;0];
            end
...
%                 end
                ...
                %% Make safe landing possible: (less than 0.1m/s linear velocity when hitting the ground.)
                % delta_z = abs(obj.X(3)-ref(3));
