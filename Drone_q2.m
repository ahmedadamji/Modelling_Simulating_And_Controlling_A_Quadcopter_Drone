...
%         plt_zdot
    ...
    methods
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function obj = Drone_q2(axis1, axis2, axis3, spaceDim, num_drones)
            if nargin > 1
                obj.axis = axis1;
...
%             hold(obj.posdot_axis,'off');
        ...
        function obj = change_pos_and_orientation(obj)
            ...
            X = [obj.pos;obj.posdot;obj.theta;obj.omega];
            deltaX =  X - obj.x_0; %computing the deltaX, small pertubations around operating point
            deltaU = transpose(obj.i) - obj.u_0; %computing deltaU

            new_X =  (obj.disc_sys.A * deltaX) + (obj.disc_sys.B * deltaU) ;%Predicting the next change/small pertubations
            
            %Getting the required inputs, used to control the quadcopter which are the square of quadcopter angular velocities.
            obj.i = calculate_inputs(cos_theta, cos_phi, obj.m, obj.g, obj.k, obj.time);%update input
            
            %Updating Quadcopter states
            obj.pos = new_X(1:3) + obj.x_0(1:3);
            obj.posdot = new_X(4:6) + obj.x_0(4:6);
            obj.theta = new_X(7:9) + obj.x_0(7:9);
            obj.thetadot = omega2thetadot(new_X(10:12),obj.theta) + obj.x_0(10:12);
